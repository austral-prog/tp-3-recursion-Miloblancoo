...
public class RecursiveFunctions {
    ...
    public static int recursiveIndexOf(List<String> list, String target) {
        ...
        int indiceEnElResto = recursiveIndexOf(list.subList(1, list.size()), target);

        if (indiceEnElResto == -1) {
            return -1;
        }
        return 1 + indiceEnElResto;
    }
    ...
    public static int recursiveIndexOfEmpty(List<String> list) {
        ...
        int result = recursiveIndexOfEmpty(list.subList(1, list.size()));
    ...
    public static int recursivePut(String target, List<String> list) {
        ...
        int result = recursivePut(target, list.subList(1, list.size()));
        return result == -1 ? -1 : result + 1;
    }
